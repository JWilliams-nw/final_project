firebase.auth().onAuthStateChanged(async function(user) {
  ...
  for (let i=0; i<classArray.length; i++) {
    ...
    <div class="kclass py-4 text-xl w-full">
      <a id=${classId} href="#" class="taken p-2 text-sm border-2 rounded-lg text-black">${className}</a>
    </div>
  `)
  document.querySelector(`#${classId}`).addEventListener('click', async function(event) {
    ...
    document.querySelector('.userlist').innerHTML = 
     `
    <div class="kclass py-4 text-xl w-full">
      <a id=${classId} href="#" class="taken p-2 text-sm bg-gray-900 text-center text-white">${names}</a>
    </div> `
